...
Metric* ScorerFactory::buildMetric(ScorerMetricType config) const {
  if (config == ScorerMetricType::mse) {
    ...
    return new Accuracy{};
  } else if (config == ScorerMetricType::fowlkes_mallows) {
    return new FowlkesMallows();
  } else if (config == ScorerMetricType::v_measure) {
      return new VMeasure();
  } else if (config == ScorerMetricType::calinski_harabasz) {
    return new CalinskiHarabasz();
  } else if (config == ScorerMetricType::david_bouldin) {
    return new DavidBouldin();
  } else {
