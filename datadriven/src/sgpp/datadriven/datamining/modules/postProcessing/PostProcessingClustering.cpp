...
void PostProcessingClustering::postProcessing(DataSource &datasource,
  ...
  double minThreshold = clusteringModel->getFitterConfiguration().getClusteringConfig().minDensityThreshold;
  double maxThreshold = clusteringModel->getFitterConfiguration().getClusteringConfig().maxDensityThreshold;
  size_t steps = clusteringModel->getFitterConfiguration().getClusteringConfig().steps;

  if (minThreshold > maxThreshold) {
    maxThreshold = minThreshold;
  }
  double stepIncrease = (maxThreshold-minThreshold)/steps;
  ...
  visualizer.runPostProcessingVisualization(model, datasource);
}
